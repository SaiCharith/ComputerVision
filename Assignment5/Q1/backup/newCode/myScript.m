...
NUM_OF_FRAMES = 59;  % It starts reading from frame 1 till NUM_OF_FRAMES
TEMPLATE_SIZE = 40;  % (2*3+1) X (2*3+1) = 7 X 7 Window
NUM_OF_POINTS = 8;  % Number of salient features to extract
...
for t=2:NUM_OF_FRAMES
    ...
    for k=1:NUM_OF_POINTS
        ...
        if L2error > threshold || newLocation(1) < 0 || newLocation(2) < 0 || L2error == 0
            fprintf("Rejected ");
