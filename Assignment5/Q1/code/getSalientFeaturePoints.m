function salientFeaturePoints = getSalientFeaturePoints(featureType, I, NUM_OF_POINTS, TEMPLATE_SIZE)
    % 1 (b)
    % It gives x coordinate as x only not like what MATLAB does 
    [height, width] = size(I);
    featurePoints = featureType(I(TEMPLATE_SIZE+1:height-TEMPLATE_SIZE,TEMPLATE_SIZE+1:width-TEMPLATE_SIZE));
    ...
    salientFeaturePoints.Location = [salientFeaturePoints.Location(:,2), salientFeaturePoints.Location(:,1)];
end