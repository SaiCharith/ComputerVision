...
%         vector_of_transforms=[vector_of_transforms tform];
   ...
   vector_of_transforms_jitter=[vector_of_transforms_jitter tformsRT];
...
for j=2:T
    H_cum=H_cum*vector_of_transforms_jitter(j-1).T;
    out = imwarp(s(j).cdata,vector_of_transforms_jitter(j-1),'OutputView',imref2d(size(s(j).cdata)));
